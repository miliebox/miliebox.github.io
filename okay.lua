print("Thank you for using the lunasense lua!! We hope you'll have a great time.") 

print(" __________________________________________________________________________")
print("| lunasense v8.2                                                          |")
print("|                                             made by ilove.sh on discord  |")
print("|__________________________________________________________________________|")
...
local H = 'lua>tab a>'
local v = 'lua>tab b>'
local l = gui.add_listbox('lunasense', H, 7, true, { '<--Global-->', '<--Rage-->',  '<--AntiAim-->', '<--Visuals-->', '<--Misc-->', '<--Configs-->' })
...
local F = gui.add_button('lunasense Discord', v, function()
    utils.print_console('\n[lunasense] ', M:get_color())
end)
--local O = gui.add_button('lunasense Coder', v, function()
--    utils.print_console('\n[lunasense] ', M:get_color())
...
function checkinghome()
    ...
    gui.set_visible(v .. 'lunasense Discord', k == 0)
--    gui.set_visible(v .. 'lunasense Coder', k == 0)
end
...
local WU = gui.add_combo('Indicators Style', v, { 'lunasense (Pixel)', 'lunasense (New)' })
...
function watermark()
    if LU:get_bool() then
        local k, j = render.get_text_size(AU, 'lunasense')
        local s, c = render.get_text_size(AU, ' | ' .. (U:get_string() .. ' | debug build'))
        local N, m = render.get_text_size(AU, 'lunasense | ' .. (U:get_string() .. ' | https://fakecri.me/@lunasensecodes'))
        local D = 255
        ...
        render.text(AU, PU / 2 - N / 2, dU - 14, 'lunasense', render.color((M:get_color()).r, (M:get_color()).g, (M:get_color()).b, 255))
        render.text(AU, (PU / 2 - N / 2) + k, dU - 14, ' | ' .. (U:get_string() .. ' | https://fakecri.me/@lunasensecodes'), render.color(98, 98, 98, 255))
    end
end
...
{ text = 'Roll', path = 'rage>anti-aim>desync>ensure lean' }, { text = 'DA', path = 'rage>aimbot>aimbot>Target dormant' }, { text = 'AX', path = 'rage>aimbot>aimbot>Anti-exploit' }, { text = 'lunasense', path = 'lua>tab b>enabled aa' } }
local VU = function()
    local k = entities.get_entity(engine.get_local_player())
...
function skeetind()
    if fU:get_bool() then
        ...
        for k, c in pairs(eU()) do
            ...
            if c == 'lunasense' then l = render.color((M:get_color()).r, (M:get_color()).g, (M:get_color()).b, U) end
...
function gui_controller()
    ...
    local v, l = render.get_text_size(j, ' [lunasense]')
    local Y = BU:get_int()
    ...
    if XU:get_bool() then
        ...
        render.text(j, 52, T / 2 + 43, 'Version: DEV v8.4', render.color(255, 255, 255, 255))
        render.text(j, 52, T / 2 + 56, 'user: ' .. (s .. ''), render.color(255, 255, 255, 255))
        render.text(j, 52 + D, T / 2 + 56, ' [lunasense]', render.color((M:get_color()).r, (M:get_color()).g, (M:get_color()).b, F))
        if Y == 0 then
...
local DC = gui.add_combo('Clantag Type', v, { 'lunasense', 'botlucky.com', 'primordial.dev', 'rawetrip', 'onetap.cc', 'gamesense.pub', 'EZfrags.co', 'neverlose.cc', 'ev0lve.xyz', 'pandora.gg' })
...
function indicators2func()
    ...
    if QU:get_bool() then
        ...
        local U, Y = render.get_text_size(s, 'lunasense')
        for k = 1, 10, 1 do
        ...
        render.text(s, (m - U / 2) + v, D + 15, 'lunasense', render.color(255, 255, 255, 255))
        if info.fatality.can_fastfire then
...
local bC = { 'ты понимаешь что я на чердаке твою бабулю повесил своим хуем', 'Stfu dog', 'Get lunasense.Lua get good', 'я на пизде твоей матери устраивал скачки на конях', 'lunasense fatality best script', '1', 'lunasense.lua Better than luck', 'Bye Bye',
'Sleep ', 'lunasense and never lose again', 'пизда твоей матери сняла номер на моем хуе' , 'bot_kick',  'The only thing lower than your k/d ratio is your I.Q', 
'Ezzz',}
...
local uC = 0
local EC = { '', 'l->', 'lu->', 'lun->', 'luna->', 'lunaa->', 'lunaas->', 'lunaase->', 'lunaasen->', 'lunaasens->', 'lunasense', 'lunasense', 'lunaasens', 'lunaasen', 'lunaase', 'lunaas', 'lunaa', 'luna', 'lun', 'lu', 'l'}
...
menu.add = {
    ...
    path = gui.add_combo("Visual functions lunasense", "lua>tab a", path),
}
